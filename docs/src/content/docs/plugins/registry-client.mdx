...
The registry's discovery side is a public, read-only API. The `@emdash-cms/registry-client` package wraps it, so you can build a plugin directory, a search page, or a release feed outside of EmDash. The client runs anywhere `fetch` is available — Node, Workers, the browser, or an Astro site. Astro sites can use `@emdash-cms/registry-loader` to expose the same data through a live content collection.
...
The current verified handle is present when the aggregator has resolved it. Keep the DID form as a fallback and redirect it to the current handle URL when a later response supplies one. A DID and package slug remain the stable package identity when a publisher changes handles.

An exact `q` value can be a handle, DID, or either identity followed by `/slug`. For example, `@example.com/my-gallery` selects one package and `example.com` returns packages from that publisher.

## Use the Astro live loader

Install `@emdash-cms/registry-loader`, then register the loader in `src/live.config.ts`:
...
Use `getLiveCollection("plugins", { q, limit })` for bounded results and `getLiveEntry("plugins", { publisher, slug })` for one package. Pass the returned cache hint to `Astro.cache.set()` when the site has an Astro cache provider. Astro live collections do not return pagination metadata, so use `DiscoveryClient.searchPackages()` directly when the interface needs the next cursor.
