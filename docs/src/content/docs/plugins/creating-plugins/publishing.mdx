...
- A valid [`emdash-plugin.jsonc`](/plugins/creating-plugins/manifest/) with `slug`, `publisher`, `license`, an author (`author` or `authors`), a security contact (`security` or `securityContacts`), and a canonical HTTPS source repository in `repo`. When `repo` is omitted, manual publishing uses the Git `origin` or `package.json#repository`. Run `emdash-plugin validate` to confirm.
...
The command signs the source repository into the package profile so sites can verify the release before installation. It refuses to publish when no canonical HTTPS repository can be detected. If `profile setup` configured the package to require provenance, publish through the generated GitHub Actions workflow instead.
