...
2. Prepare the package profile and generate the workflow from the plugin directory.
   ...
   Setup also asks whether releases require verifiable provenance. **Require provenance** is the default for automated releases. Choose **Allow releases without provenance** only when the same profile must also accept releases published directly from a trusted local environment.

   The signed-in Atmosphere account becomes the initial approver. The profile binds the package to the canonical GitHub repository URL and records the selected provenance policy.
   ...
   In a non-interactive terminal, pass `--yes` to accept the default policies. Pass `--repository <https-url>` when neither the manifest nor Git remote supplies the repository, `--provenance optional` to permit releases without provenance, and `--confirmation always` to require approval for every release.
