...
The scaffolded test invokes the original `hello` route through the Worker Loader transport host. Replace it with a test for the `health` route:
...
- [Testing](/plugins/creating-plugins/testing/) covers direct sandbox transport tests and runtime-backed host-action tests.
