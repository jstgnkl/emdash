...
Use `ctx.settings` for user configuration and `ctx.kv` for small internal values. Both stores are isolated by plugin. Declare credentials as `secret` fields in `admin.settingsSchema` so EmDash encrypts them.
