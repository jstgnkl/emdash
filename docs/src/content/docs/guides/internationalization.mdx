...
Terms are per-locale. Each locale can have its own definition, so `label` and
...
A [reference field](/guides/relations/) is shared rather than synced: EmDash keys its links by the translation group, so every translation of an entry has one selection, and editing it from any locale changes it for all of them.
