...
Remove one of these images in the admin with its **Remove** button. API and MCP clients can remove `logo`, `favicon`, or `seo.defaultOgImage` by setting that field to `null`; omitting a field leaves its current value unchanged.
