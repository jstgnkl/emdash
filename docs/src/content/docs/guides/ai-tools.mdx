...
- **Upload files** -- "Upload this base64 image as `banner.png`"
...
`media_upload` accepts base64-encoded bytes, stores the file, registers the media item, and returns the ID and URL. It enforces the site's MIME-type and upload-size limits.
