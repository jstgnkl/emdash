...
Let editors pick one page template, for example Default, Full Width, or Landing Page. A `select` field stores the choice, and the page route maps that value to an Astro layout component. Use a [`blocks` field](/guides/blocks/) when editors need to compose and reorder several sections within one page.
