...
## Add structured page blocks

Use a [`blocks` field](/guides/blocks/) when the page itself is an ordered composition of typed sections. Use a custom Portable Text block when the object belongs inside a rich-text document and participates in its paragraph flow.

The marketing template seeds five retained block types and allows them in the Pages `content` field. Its wrapper maps every generated `_type` to an Astro component:
...
The generated `PageContentBlock` union keeps the component map aligned with every allowed block type and retained version. Each seeded block value includes `_type`, `_version`, and a stable `_key`.

Use a custom Portable Text block for an object that belongs between paragraphs, such as an embedded diagram in an article. Map that object through the `PortableText` component's `components.type` option. A seed can include the stored value, but a native plugin must register a custom editor for creating and editing that Portable Text object.
...
6. Edit site settings, menus, taxonomy assignments, Portable Text, and blocks through the admin. For blocks, add, reorder, duplicate, and remove each allowed type. Confirm the public routes use the changed values.
