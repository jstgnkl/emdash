...
A [`reference` field](/reference/field-types/#reference) links an entry to entries in another collection through a relation. Its value is not part of `data`, and it is keyed by translation group, so every translation of an entry shares one selection.

Creation and update bodies carry selections under `references`, keyed by field slug, each an array of at most 1000 entry IDs in display order. EmDash writes the selection in the same transaction as the entry. The following update replaces the entry's author:
...
A field bound to the child end of its relation selects the entries pointing at the one being written, which carry no order of their own. Relation limits are enforced on both ends, so a selection that would give a linked entry more parents than the relation allows is rejected alongside one that links too many entries itself.

On a collection that keeps revisions, a selection changed on a published entry is staged in the draft with the entry's other pending edits. It becomes live when the entry is published and is discarded with the draft. Publishing re-checks the whole selection against the relation's limits.

A single-entry read returns `references` keyed by field slug. Each field holds the first page of its linked entries, 50 of them, with a `nextCursor` when the field holds more, and reports each entry's ID, slug, collection, display title, resolved locale, and translation group. A caller who may read drafts sees a staged selection where the draft carries one. The content list operation does not include references.

Relation definitions and link traversal are admin routes, absent from OpenAPI and outside the public contract. Write and read selections through the content operations above, and render them on a site with [`getEmDashEntry()`](/reference/api/#getemdashentry) and [`getEmDashReferences()`](/reference/api/#getemdashreferences).
